size.plotting.all.gamma <- function(distribution="normal", max.sample.size=25, increments=2, nb.of.pvalues.each=100, c_N1=0.5, c_N2=0.5){
  ...
  }else if(distribution=="kappa4"){
    #sampling pvalues from appropriate distribution nb.of.pvalues.each times per sample size
    p.values.for.gamma1 = lapply(m.vector, sampling.pvalues.from.kappa4.standard.normal, gamma=gamma1, repeats=nb.of.pvalues.each, c_N1=c_N1, c_N2=c_N2)
    p.values.for.gamma2 = lapply(m.vector, sampling.pvalues.from.kappa4.standard.normal, gamma=gamma2, repeats=nb.of.pvalues.each, c_N1=c_N1, c_N2=c_N2)
    p.values.for.gamma3 = lapply(m.vector, sampling.pvalues.from.kappa4.standard.normal, gamma=gamma3, repeats=nb.of.pvalues.each, c_N1=c_N1, c_N2=c_N2)  
    p.values.for.gamma4 = lapply(m.vector, sampling.pvalues.from.kappa4.standard.normal, gamma=gamma4, repeats=nb.of.pvalues.each, c_N1=c_N1, c_N2=c_N2)
    p.values.for.gamma5 = lapply(m.vector, sampling.pvalues.from.kappa4.standard.normal, gamma=gamma5, repeats=nb.of.pvalues.each, c_N1=c_N1, c_N2=c_N2)
...
color.vector = gray.colors(5, start = 0, end = 0.4, gamma = 2.2)
  ...
  if(distribution=="normal"){ 
    ...
    plot(m.vector, size.vector.gamma1, ylim=c(0, 0.10), xlab="m (first sample size)", ylab="Size (at alpah=0.05)",main="Sizes each for 2000 pvalues (mv laplace)", sub="(c_N1=0.1 and c_N2=0.1)", lty=1, col=color.vector[1], type="l")
  }else if(distribution=="kappa4"){
    plot(m.vector, size.vector.gamma1, ylim=c(0, 0.10), xlab="m (first sample size)", ylab="Size (at alpah=0.05)",main="Sizes each for 2000 pvalues (MEP; kappa=4)", sub="(c_N1=0.1 and c_N2=0.9)", lty=1, col=color.vector[1], type="l")
